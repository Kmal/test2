...
esp_err_t board_power_read_status(board_power_status_t *out_status)
{
    ...
    if (m5pm1_read_vin_mv(BOARD_I2C_PORT, BOARD_M5PM1_ADDR, &vin_mv) == ESP_OK) {
        out_status->vin_mv = vin_mv;
    }

    if (m5pm1_read_5v_inout_mv(BOARD_I2C_PORT, BOARD_M5PM1_ADDR, &fivev_mv) == ESP_OK) {
    ...
    out_status->usb_present =
        vin_mv >= s_board_power_config.usb_present_mv_threshold ||
        fivev_mv >= s_board_power_config.usb_present_mv_threshold;
