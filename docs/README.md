...
- Added Wi-Fi station/setup-AP provisioning, LCD keyboard provisioning, a local web UI, JSON APIs, NVS-backed automation config storage, and capability reporting. The Web UI resource budget is documented in `docs/web_ui_resource_budget.md`, the phased redesign status is tracked in `docs/web_ui_redesign_status.md`, and generated static assets are checked by `tools/check_web_ui_budget.py` during host validation.
