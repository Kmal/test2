...
The StickS3 documentation and schematic identify M5PM1/PY `G2` as `PYG2_L3B_EN`, with the ES8311 audio rail on `3V3_L3B_AU`. The default capture-only boot now enables that rail before ES8311 register access by configuring M5PM1 GPIO2 as a normal output, push-pull drive, and **high** output using the same tested GPIO-helper path as LCD/L3B setup. The current M5Stack M5GFX StickS3 initialization source drives `PYG2` high to enable L3B, writes `I2C_CFG=0x00`, and waits for the rail before LCD reset. M5PM1 powers up in 100 kHz I2C mode, so the shared register-bus cache keeps the M5PM1 device handle at 100 kHz while other devices can use the board-default 400 kHz speed. The PMIC helper retries a first `ESP_ERR_INVALID_RESPONSE` once, because the boot log showed the LCD path failing on the first M5PM1 GPIO-function read while a later L3B access succeeded. The separate M5PM1 identity probe remains optional, but an L3B power-enable failure is fail-fast because ES8311 I2C access will not be reliable while the audio rail is unavailable.
