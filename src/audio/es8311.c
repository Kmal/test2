 ...
 * a DAC-only profile for speaker actions, and an ADC+DAC profile for explicit
 * USB Audio Class simultaneous mic+speaker experiments. Future changes must classify
                              ...
                              es8311_profile_t profile, int sample_rate)
{
    ...
    if (profile != ES8311_PROFILE_ADC_ONLY &&
        ...
        profile != ES8311_PROFILE_ADC_DAC) {
    ...
    const char *profile_name = profile == ES8311_PROFILE_ADC_ONLY ? "adc-only" :
                               (profile == ES8311_PROFILE_DAC_ONLY ? "dac-only" : "adc-dac");
    (void)profile_name;
