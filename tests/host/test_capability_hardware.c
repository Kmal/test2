...
int main(void)
{
    ...
    assert(capability_source_runtime_available(RULE_SOURCE_BATTERY_PERCENT) == (bool)CONFIG_APP_POWER_FACTS);
    assert(capability_source_runtime_available(RULE_SOURCE_POWER_USB_PRESENT) == (bool)CONFIG_APP_POWER_FACTS);
    ...
    if (CONFIG_APP_POWER_FACTS) {
        assert(strcmp(capability_source_availability_reason(RULE_SOURCE_BATTERY_PERCENT), "implemented") == 0);
    } else {
        assert(strcmp(capability_source_availability_reason(RULE_SOURCE_BATTERY_PERCENT), "power_facts_disabled") == 0);
    }
    return 0;
}
