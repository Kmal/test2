...
static void test_adc_dac_profile_configures_capture_and_playback(void)
{
    fake_register_bus_reset();
    ASSERT_EQ(ESP_OK, es8311_init_profile(I2C_NUM_0, 0x18, I2S_NUM_0, ES8311_PROFILE_ADC_DAC, 16000));
    ASSERT_TRUE(fake_register_bus_has_write(0x18, 0x0a, 0x0c));
    ASSERT_TRUE(fake_register_bus_has_write(0x18, 0x14, 0x1a));
    ASSERT_TRUE(fake_register_bus_has_write(0x18, 0x17, 0xc8));
    ASSERT_TRUE(fake_register_bus_has_write(0x18, 0x09, 0x0c));
...
int main(void)
{
    ...
    test_adc_dac_profile_configures_capture_and_playback();
