...
static void test_adc_only_does_not_power_or_unmute_dac(void)
{
    fake_register_bus_reset();
    ASSERT_EQ(ESP_OK, es8311_init_profile(I2C_NUM_0, 0x18, I2S_NUM_0, ES8311_PROFILE_ADC_ONLY, 16000));
    ASSERT_TRUE(!fake_register_bus_has_write(0x18, 0x15, 0x06));
    ASSERT_TRUE(!fake_register_bus_has_write(0x18, 0x31, 0x00));
    ASSERT_TRUE(fake_register_bus_has_write(0x18, 0x15, 0x00));
...
int main(void)
{
    ...
    test_adc_only_does_not_power_or_unmute_dac();
