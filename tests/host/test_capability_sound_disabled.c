...
int main(void)
{
    ...
    ASSERT_TRUE(strcmp(capability_source_availability_reason(RULE_SOURCE_SOUND_RMS_DBFS), "audio_capture_disabled") == 0);
    ASSERT_TRUE(capability_build_json(json, sizeof(json)) > 0);
    ASSERT_TRUE(strstr(json, "sound.rms_dbfs") != NULL);
    ASSERT_TRUE(strstr(json, "\"runtime_available\":false") != NULL);
    ASSERT_TRUE(strstr(json, "audio_capture_disabled") != NULL);
    puts("capability_sound_disabled tests passed");
    return 0;
}
